...
const handleLeaderboardJSON = () => {
  namesArray = leaderboard;
  // init after data ready && carOject ready
  let timeInterval = setInterval(() => {
    if (Object.keys(carObject).length) {
      console.log("init");
      init();
      animate();
...
function loader() {
  ...
  for (let i = 0; i < 3; i++) {
    mtlLoader.load(`static/Low-Poly-Racing-Car-${i}.mtl`, (mtlParseResult) => {
      const objLoader = new OBJLoader2();
      const materials = MtlObjBridge.addMaterialsFromMtlLoader(mtlParseResult);
      objLoader.addMaterials(materials);
      objLoader.load("static/Low-Poly-Racing-Car.obj", (root) => {
        root.position.y = -10;
